...
class TestFinishMethod:
    ...
    async def test_finish_with_documents_in_different_states(self, pipeline_docstore):
        ...
        for original_doc in mixed_docs:
            if original_doc.state == "link":
                ...
                download_children = [
                    pipeline_docstore.get(id=child_id)
                    for child_id in stored_doc.children
                ]
    ...
    @pytest.mark.asyncio
    async def test_finish_with_error_handling(
        self, sqlite_connection_string, pipeline_document_type
    ):
