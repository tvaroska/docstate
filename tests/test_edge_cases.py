...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_no_documents_in_final_state(
    ...
    def test_get_state_returns_single_document(
        self, sqlite_connection_string, document_states
    ):
        ...
        doc_type = DocumentType(states=[link, download], transitions=[])
    ...
    @pytest.mark.asyncio
    async def test_next_with_valueerror_exception(
        self, sqlite_connection_string, document_states
    ):
        ...
        doc_type = DocumentType(
            ...
            transitions=[
                Transition(
                    from_state=link, to_state=download, process_func=problem_process
                )
            ],
        )
