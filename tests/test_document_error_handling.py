...
class TestDocumentErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_failed_transition_creates_error_document(self, error_handling_docstore):
    ...
    @pytest.mark.asyncio
    async def test_custom_error_state(self, sqlite_connection_string, error_handling_document_type):
    ...
    @pytest.mark.asyncio
    async def test_list_input_with_mixed_success_and_failure(self, document_type, error_handling_document_type, sqlite_connection_string):
        ...
        async def conditional_failing_process_func(doc: Document) -> Document:
            if "fail" in doc.content:
                raise ValueError(f"Failed processing document with content: {doc.content}")
            return Document(
        ...
        docstore = DocStore(connection_string=sqlite_connection_string, document_type=mixed_type)
        
        # Create documents - some will fail, some will succeed
        doc1 = Document(media_type="text/plain", state="link", content="normal document")
        doc2 = Document(media_type="text/plain", state="link", content="fail this document")
        doc3 = Document(media_type="text/plain", state="link", content="another normal document")
