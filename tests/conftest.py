...
def mock_process_functions():
    ...
    async def mock_download(doc):
        return Document(
            content="Downloaded content", media_type="text/plain", state="download"
        )
    ...
    async def mock_embed(doc):
        return Document(
            content="Embedded vector", media_type="application/vector", state="embed"
        )

    return {"download": mock_download, "chunk": mock_chunk, "embed": mock_embed}
...
def document_type(document_states, transitions):
    ...
    """
    return DocumentType(
        states=list(document_states.values()), transitions=list(transitions.values())
    )
...
def root_document():
    ...
    """
    return Document(media_type="text/plain", state="link")
...
def child_document(root_document):
    ...
    """
    return Document(
        media_type="text/plain", state="download", parent_id=root_document.id
    )
...
def document_with_children():
    ...
    """
    return Document(
        media_type="text/plain", state="download", children=[str(uuid4()), str(uuid4())]
    )
