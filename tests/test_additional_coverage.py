...
def document_type_with_no_docstore_errors(document_states, process_func):
    ...
    async def failing_process(doc: Document) -> Document:
        ...
        return Document(content="Success content", state="download")

    return DocumentType(
        states=[link, download, chunk, embed],
        transitions=[
            Transition(
                from_state=link, to_state=download, process_func=failing_process
            ),
...
class TestAdditionalCoverage:
    ...
    @pytest.mark.asyncio
    async def test_next_with_general_exception(
        ...
        doc_type = DocumentType(
            ...
            transitions=[
                Transition(
                    from_state=link, to_state=download, process_func=broken_process
                )
            ],
        )
    ...
    @pytest.mark.asyncio
    async def test_finish_with_next_returning_no_docs(
        self, sqlite_connection_string, document_type_with_no_docstore_errors
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_by_state_single_result(
        self, sqlite_connection_string, document_type_with_no_docstore_errors
    ):
